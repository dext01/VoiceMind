...
class SetupActivity : AppCompatActivity() {
    ...
    companion object {
        private const val MODEL_NAME = "gemma-3-1b-it-Q4_K_M.gguf"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (modelExists()) {
            ...
            // Показываем экран с выбором
            tvStatus.text   = "Модель LLM для улучшения текста не найдена"
            tvProgress.text = "Можно скачать сейчас или пропустить и использовать только алгоритмы"
    ...
    private fun startDownload() {
        ...
        Thread {
            val dest = File(filesDir, MODEL_NAME)
            val tmp  = File(filesDir, "$MODEL_NAME.tmp")
            try {
                val ok = download(MODEL_URL, tmp) { pct, dlMb, totalMb ->
                    mainHandler.post {
                        progressBar.progress = pct
                        tvStatus.text        = "Загрузка Gemma 3 1B..."
                ...
                if (ok) {
                    ...
                    tmp.delete()
                    mainHandler.post { showError("Ошибка загрузки. Проверьте интернет.") }
                }
            } catch (e: Exception) {
                tmp.delete()
                mainHandler.post { showError("Ошибка: ${e.message}") }
            }
        }.start()
    }
    ...
    private fun download(
        startUrl: String,
        dest:     File,
        onProgress: (pct: Int, dlMb: Long, totalMb: Long) -> Unit,
    ): Boolean {
        var urlStr = startUrl
        for (i in 0 until 10) {
